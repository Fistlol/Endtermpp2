...
loose = mixer.Sound("Music/loose.wav")
...
class Bullet:
    ...
    def bullet_pos(self):
        if self.direction == Direction.RIGHT and self.destroyed == False:
            self.bullet_x = self.tank.x + self.tank.width + self.tank.width // 2
            self.bullet_y = self.tank.y + self.tank.width // 2
            self.bullet_width = 20
            self.bullet_height = 4
        if self.direction == Direction.LEFT and self.destroyed == False:
            self.bullet_x = self.tank.x - self.tank.width // 2
            self.bullet_y = self.tank.y + self.tank.width // 2
            self.bullet_width = 20
            self.bullet_height = 4
        if self.direction == Direction.UP and self.destroyed == False:
            self.bullet_x = self.tank.x + self.tank.width // 2
            self.bullet_y = self.tank.y - self.tank.width // 2
            self.bullet_width = 4
            self.bullet_height = 20
        if self.direction == Direction.DOWN and self.destroyed == False:
            self.bullet_x = self.tank.x + self.tank.width // 2
...
tank0 = Tank((125, 125, 125), (125, 105, 125), 60)
tank1 = Tank((0, 180, 64), (0, 150, 64), 740, pygame.K_d, pygame.K_a, pygame.K_w, pygame.K_s)
...
while game:
    ...
    if bullet0.collision_tank(tank1):
        if music_loose == False:
            hit.play()
        tank1.random_pos()
        tank1.life_count -= 1
        bullet0.bullet_x = -100
        bullet0.bullet_y = -100
        bullet0.destroyed = False
